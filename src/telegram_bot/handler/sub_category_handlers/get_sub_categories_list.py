...
class GetSubCategoriesListHandler(BaseCommandHandler):
    ...
    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_sub_categories_list")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetSubCategoriesListHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_sub_categories_list'):
                instructions = (
