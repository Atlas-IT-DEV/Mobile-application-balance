...
class GetHistoryPaymentByFeeUserHandler(BaseCommandHandler):
    CHOOSING, WAITING_FOR_PHOTO = range(2)

    FORMA = ("<pre>"
             "/get_history_payment_by_fee_user\n"
             "fee_id=ID\n"
             "user_id=ID"
             "</pre>\n\n")

    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_history_payment_by_fee_user")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetHistoryPaymentByFeeUserHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_history_payment_by_fee_user'):
                instructions = (
            ...
            if self.DATA[user_id]:
                ...
                response = requests.get(
                    f'http://{self.HOST}:{self.SERVER_PORT}/history_payments/fee_id/{self.DATA[user_id]["fee_id"]}/user_id/{self.DATA[user_id]["user_id"]}',
                )
                ...
                for message in messages:
                    await _inf_response(update, response,
                                        f"{message}",
                                        f"Ошибка при получении списка категорий сборов: \n{response.text}")
