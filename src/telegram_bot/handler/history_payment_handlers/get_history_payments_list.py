...
class GetHistoryPaymentsListHandler(BaseCommandHandler):
    CHOOSING, WAITING_FOR_PHOTO = range(2)

    FORMA = ("<pre>"
             "/get_history_payments_list"
             "</pre>\n\n")

    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_history_payments_list")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetHistoryPaymentsListHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_history_payments_list'):
                instructions = (
