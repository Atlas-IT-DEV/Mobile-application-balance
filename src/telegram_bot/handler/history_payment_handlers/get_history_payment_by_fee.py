...
class GetHistoryPaymentByFeeHandler(BaseCommandHandler):
    CHOOSING, WAITING_FOR_PHOTO = range(2)

    FORMA = ("<pre>"
             "/get_history_payment_by_fee\n"
             "fee_id=ID"
             "</pre>\n\n")

    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_history_payment_by_fee")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetHistoryPaymentByFeeHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_history_payment_by_fee'):
                instructions = (
            ...
            if self.DATA[user_id]:
                ...
                response = requests.get(
                    f'http://{self.HOST}:{self.SERVER_PORT}/history_payments/fee_id/{self.DATA[user_id]["fee_id"]}',
                )
