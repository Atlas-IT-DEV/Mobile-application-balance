...
class CreateFeeCategoryHandler(BaseCommandHandler):
    ...
    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command create_fee_category")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Напиши команду в формате:</b>\n"
                    f"{CreateFeeCategoryHandler.FORMA}"
                    "<i>На улице дождь?</i>"
                )
                await update.message.reply_text(instructions, parse_mode='HTML')
                return self.CHOOSING
        except Exception as ex:
            log.exception(f"Failed method: {ex}")    # Проверяем существоание пользователя
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/create_fee_category'):
                instructions = (
            ...
            if self.DATA[user_id]:
                ...
                fee_category_data = FeeCategories(name=self.DATA[user_id]["name"]).dict(by_alias=True)

                response = requests.post(
                    f'http://{self.HOST}:{self.SERVER_PORT}/fee_categories/',
