...
class GetFeeCategoriesListHandler(BaseCommandHandler):
    CHOOSING, WAITING_FOR_PHOTO = range(2)

    FORMA = ("<pre>"
             "/get_fee_categories_list"
             "</pre>\n\n")

    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_fee_categories_list")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetFeeCategoriesListHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_fee_categories_list'):
                instructions = (
