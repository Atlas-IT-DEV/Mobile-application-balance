...
class GetSubscriptionByUserHandler(BaseCommandHandler):
    ...
    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_subscription_by_user")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetSubscriptionByUserHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_subscription_by_user'):
                instructions = (
            ...
            response = requests.get(
                f'http://{self.HOST}:{self.SERVER_PORT}/subscriptions/user_id/{self.DATA[user_id]["user_id"]}',
            )
