...
class GetSubscriptionByFeeHandler(BaseCommandHandler):
    CHOOSING, WAITING_FOR_PHOTO = range(2)

    FORMA = ("<pre>"
             "/get_subscription_by_fee\n"
             "fee_id=ID"
             "</pre>\n\n")

    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_subscription_by_fee")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetSubscriptionByFeeHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_subscription_by_fee'):
                instructions = (
            ...
            response = requests.get(
                f'http://{self.HOST}:{self.SERVER_PORT}/subscriptions/fee_id/{self.DATA[user_id]["fee_id"]}',
            )
            ...
            for message in messages:
                await _inf_response(update, response,
                                    f"{message}",
                                    f"Ошибка при получении списка подписок: \n{response.text}")
