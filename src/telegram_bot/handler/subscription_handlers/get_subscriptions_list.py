...
class GetSubscriptionsListHandler(BaseCommandHandler):
    ...
    async def start(self, update: Update, context: CallbackContext) -> int:
        log.info("Command get_subscriptions_list")
        try:
            if await self.check_authorized(update):
                instructions = (
                    "<b>Подтвердите ввод команды:</b>\n"
                    f"{GetSubscriptionsListHandler.FORMA}"
    ...
    async def handle_message(self, update: Update, context: CallbackContext) -> int:
        ...
        try:
            ...
            if not text.startswith('/get_subscriptions_list'):
                instructions = (
