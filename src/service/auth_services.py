...
from src.database.models import User, TokenInfo, Auth
...
async def validate_auth_user(auth: Auth):
    user = user_services.get_user_by_phone(auth.Phone)
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid phone")
    if not validate_password(password=auth.Password, hashed_password=user.Password.encode("utf-8")):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid password")
    return user


async def validate_reg_user(user: User):
    ...
    existing_user = user_services.get_user_by_phone(user.Phone)
    if existing_user:
...
async def auth_refresh_jwt(user: User):
    return TokenInfo(access_token=create_access_token(user))
